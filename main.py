...
def transformImage( oldImage, newImage, forwardTransform ):
    ...
    inverse_T = np.linalg.inv(forwardTransform)

    # use backward projection
    for x in range(width):
      for y in range(height):
        new = np.array([x], [y], [1])
        old = np.dot(inverse_T, new)
        x = old[0][0]
        y = old[1][0]
        # check if it is outside of the image
        try:
    ...
    # use bilinear interpolation to put new values into dstPixels
